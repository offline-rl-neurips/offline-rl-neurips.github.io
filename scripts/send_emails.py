...
def get_zoom_meetings(papers):
    meetings = []
    for unique_id in papers["unique_id"].unique():
        if meeting_json_exists("BAICS_{}".format(unique_id)):
            meeting_info = read_meeting_json("BAICS_{}".format(unique_id))
...
def get_meeting_data():
    """Return a list of dicts with meeting data."""
    papers = load_presentation_data()
    meetings = get_zoom_meetings(papers)
